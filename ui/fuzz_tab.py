...
class FuzzTab(QWidget):
    def __init__(self):
        ...
        self.url_input.setPlaceholderText("如: http://example.com/FUZZ")
        self.basic_layout.addRow("目标 URL(-u):", self.url_input)
        ...
        self.basic_layout.addRow("字典文件(-w):", wordlist_layout)

        # 线程数 (-t)
        self.threads_input = QSpinBox()
        self.threads_input.setRange(1, 100)
        self.threads_input.setValue(10)
        self.basic_layout.addRow("线程数(-t):", self.threads_input)
        ...
        # HTTP 设置: method(-m), useragent(-a), follow-redirect(-r), random-agent, no-tls-validation(-k), proxy
        ...
        self.http_group_layout.addRow("请求Body(-B):", self.body_input)
        ...
        # 长度过滤: --exclude-length
        self.status_group = QGroupBox("过滤设置")
        ...
        self.status_group_layout.addRow("排除长度(--exclude-length):", self.exclude_length_input)

        # 状态码排除(-b)
        self.excludestatuscodes_input = QLineEdit()
        self.status_group_layout.addRow("排除状态码(-b):", self.excludestatuscodes_input)
        ...
        # Headers, Cookies, no-canonicalize-headers
        self.other_group = QGroupBox("请求头与Cookies")
        ...
        self.other_group_layout.addRow("HTTP头(-H,逗号分隔):", self.headers_input)

        self.no_canonicalize_input = QCheckBox("不标准化头(--no-canonicalize-headers)")
    ...
    def bind_signals(self):
        ...
        self.body_input.textChanged.connect(self.update_command)

        # 排除长度、状态码
        self.exclude_length_input.textChanged.connect(self.update_command)
        self.excludestatuscodes_input.textChanged.connect(self.update_command)
    ...
    def update_command(self):
        ...
        if "linux" in sys.platform:
            command = [os.path.join(directory_path, 'gobuster'), "fuzz"]
        elif "win" in sys.platform:
            command = [os.path.join(directory_path, 'gobuster.exe'), "fuzz"]
        ...
        if self.advanced_box.toggle_button.isChecked():
            ...
            if self.body_input.text():
                command.extend(["-B", self.body_input.text()])

            if self.exclude_length_input.text():
                command.extend(["--exclude-length", self.exclude_length_input.text()])
            if self.excludestatuscodes_input.text():
                command.extend(["-b", self.excludestatuscodes_input.text()])

        # 额外设置
        if self.extra_box.toggle_button.isChecked():
            # Headers, Cookies, no-canonicalize
